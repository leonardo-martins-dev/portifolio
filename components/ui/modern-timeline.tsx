...
const getStatusConfig = (status: TimelineItem["status"]) => {
  const configs = {
    ...
    current: {
      ...
      badgeText: "text-white dark:text-blue-200"
    },
    upcoming: {
  ...
  return (
    ...
    >
      <div className="relative">
        <div 
          className="absolute left-4 sm:left-6 top-0 bottom-0 w-px bg-gray-700" 
          aria-hidden="true"
        />
        
        <motion.div
          className="absolute left-4 sm:left-6 top-0 w-px bg-gradient-to-b from-purple-500 to-orange-400 origin-top"
          initial={{ scaleY: 0 }}
          whileInView={{ 
            scaleY: 1,
            transition: {
                    ...
                    >
                      <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full overflow-hidden border-2 border-background shadow-lg relative z-10">
                        {item.image ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-r from-purple-500 to-orange-400 flex items-center justify-center">
                            <IconComponent 
                      ...
                      <CardContent className="p-4 sm:p-6">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-3">
                          <div className="flex-1 min-w-0">
                            <motion.h3 
                              className="text-lg sm:text-xl font-semibold text-white mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-purple-500 group-hover:to-orange-400 transition-colors duration-300"
        ...
        <motion.div
          ...
          aria-hidden="true"
        >
          <div className="w-3 h-3 bg-gradient-to-r from-purple-500 to-orange-400 rounded-full shadow-sm" />
        </motion.div>
      </div>
    </section>
  )
